...
BEGIN
  -- Strip country code prefix to get last 10 digits
  ...
  -- Backend validation: reject invalid Indian mobile numbers
  ...
  -- Fixed source-to-owner routing (no round-robin for mapped sources)
  CASE _lead_source_type::text
    ...
    WHEN 'sandbox_access_request' THEN _assignee := _uid_lakshay;
    WHEN 'production_access_request' THEN _assignee := _uid_lakshay;
    WHEN 'integration_query' THEN _assignee := _uid_lakshay;
    ELSE
      -- Fallback: round-robin for unmapped sources (excel_import, manual_entry, etc.)
  ...
  -- Check for insurance-related leads (override if message/metadata hints at insurance)
  IF _insurance_flag = true AND _assignee IS DISTINCT FROM _uid_ayushi THEN
    -- If insurance is the primary intent, route to Ayushi
  ...
  IF _existing_id IS NOT NULL THEN
    -- MERGE: preserve existing owner (never reassign on resubmission)
    UPDATE public.leads
    SET
      ...
      -- Preserve existing owner; only fill if previously null
