...
import { fetchLeads, leadsToCSV, type LeadRow, type LeadStatus, type LeadSourceType, type AudienceType } from "@/lib/leads-service";
...
const SOURCE_OPTIONS: { value: LeadSourceType; label: string }[] = [
  ...
  { value: "sandbox_access_request", label: "Sandbox Request" },
  { value: "production_access_request", label: "Production Request" },
  { value: "demo_request", label: "Demo Request" },
  { value: "support_request", label: "Support" },
  { value: "integration_query", label: "Integration Query" },
];
...
const VIEW_PRESETS: { label: string; filter: Partial<{ status: LeadStatus; sourceType: LeadSourceType; audience: AudienceType }> }[] = [
  ...
  { label: "Demo Requests", filter: { sourceType: "demo_request" } },
  ...
  { label: "Closed Lost", filter: { status: "closed_lost" } },
];
...
const OpsLeads = () => {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(false);
  }, [search, statusFilter, sourceFilter, audienceFilter, page]);
  ...
  const handleExport = async () => {
    const all = await fetchLeads({ search, status: statusFilter || undefined, sourceType: sourceFilter || undefined, audience: audienceFilter || undefined, pageSize: 10000 });
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <Select value={sourceFilter} onValueChange={(v) => { setSourceFilter(v as LeadSourceType); setPage(1); setActivePreset(""); }}>
            <SelectTrigger className="w-[170px] h-9 text-xs"><SelectValue placeholder="Source" /></SelectTrigger>
            <SelectContent>
          ...
          <Select value={audienceFilter} onValueChange={(v) => { setAudienceFilter(v as AudienceType); setPage(1); setActivePreset(""); }}>
            <SelectTrigger className="w-[140px] h-9 text-xs"><SelectValue placeholder="Audience" /></SelectTrigger>
            <SelectContent>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-left px-3 py-2.5 text-xs font-medium text-muted-foreground hidden md:table-cell">Mobile</th>
                  <th className="text-left px-3 py-2.5 text-xs font-medium text-muted-foreground hidden lg:table-cell">Email</th>
                  <th className="text-left px-3 py-2.5 text-xs font-medium text-muted-foreground hidden lg:table-cell">Company</th>
                  <th className="text-left px-3 py-2.5 text-xs font-medium text-muted-foreground">Source</th>
                  <th className="text-left px-3 py-2.5 text-xs font-medium text-muted-foreground">Audience</th>
              ...
              <tbody>
                ...
                {!loading && leads.map((lead) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 text-xs hidden md:table-cell">{lead.mobile_number ?? "—"}</td>
                    <td className="px-3 py-2.5 text-xs hidden lg:table-cell">{lead.email ?? "—"}</td>
                    <td className="px-3 py-2.5 text-xs hidden lg:table-cell">{lead.company_name ?? "—"}</td>
                    <td className="px-3 py-2.5 text-xs capitalize whitespace-nowrap">{lead.lead_source_type?.replace(/_/g, " ") ?? "—"}</td>
                    <td className="px-3 py-2.5 text-xs capitalize">{lead.audience_type ?? "—"}</td>
